...
### Kindle reading-session import (codified 2026-05-10, takeout arrived 2026-05-12)
...
- **Takeout-arrival findings (2026-05-12):** takeout at `~/tmp/kindle/`. `Kindle.Devices.ReadingSession.csv` carries **12,957 sessions across 231 unique ASINs from 2017→2026** (~3,495 total reading hours). Mostly Kindle E-reader (10,205 sessions), some Cloud Reader (1,394), a little Manga Reader (313); 386 sessions have malformed/missing timestamps and need a skip-rule. Crucially, the takeout also includes `Digital.Content.Ownership/*.json` (980 shards, one per owned title) carrying `ASIN`, `Product Name`, `acquiredDate` — **this kills the Calibre dependency entirely**; ASIN→title comes from Amazon directly. No author field in the ownership shards, so the vault-match leans on title (+ vault author as a tiebreaker via `goodreads_id` / `hardcover_slug` lookups where present).

The value: **one-shot recovery of behavioural data we can't reconstruct later.** Hardcover/Open Library backfills aggregate other people's signal; this is the user's own reading-session history stretching back to first Kindle ownership. Different category — file the takeout request, ingest once, future re-requests are optional.

- **`scripts/import-kindle-sessions.mjs`**: read `Kindle.Devices.ReadingSession.csv` + `Digital.Content.Ownership/*.json` from a takeout root (default `~/tmp/kindle/`, override via `--takeout`), join on ASIN, emit `_meta/kindle-sessions.json` **keyed by ASIN** with shape `{ asin → { title, acquiredDate, sessions: [{ start, end, durationSeconds, pageFlips, device }] } }`. Pure ingestion — no vault concerns, no slug join here. Operator-run via `make vault-kindle-sessions`. Dry-run by default with prompt-to-apply. Cache committed to the vault; build stays offline-clean. `#integration #kindle #amazon`
- **`amazon_asin` frontmatter field + Ownership-driven backfill**: most vault books don't carry an ASIN today; without it session rows have nothing to link to. Add `amazon_asin: <string>` to the schema (`src/lib/types.ts`, `scripts/build-index.mjs` parse, the walk-fallback parser in `src/lib/books.ts`, `_meta/CLAUDE.md` in the vault), then `scripts/backfill-asin-from-sessions.mjs` that reads `_meta/kindle-sessions.json`, matches `Product Name` against vault titles (case-insensitive, punctuation-normalised; tiebreak on author when the vault entry already carries Hardcover/Goodreads metadata), and prompts per book before writing. Pure cache-to-frontmatter, no network. `#schema #vault #asin`
