...
- Expand component-test coverage further. `DeepNotes` and `Cover` are covered now; still wanted is an integration shape for the per-book server component itself, so the tier-0/tier-1/tier-2 reveal contract is end-to-end-pinned rather than per-component-pinned. `#testing`
...
- **Multi-series schema (books in more than one series).** Discworld is the canonical example: every Witches/Watch/Tiffany Aching book is also a Discworld book, with its own `#N` per series. The vault already encodes this informally: some entries carry strings like `series: "Discworld, #32; Tiffany Aching #2"` and `series: "Discworld, #12; Witches #3"`, semicolon-delimited. **Both parsers butcher it.** `parseSeriesField` (the renderer path for `/series`) treats the whole thing as one series called `"Discworld, #32; Tiffany Aching"` with index `2`, so multi-series books land in their own oddball bucket separate from singleton-Discworld. `stripSeriesIndex` (the backfill path) returns `"Discworld,"` (with trailing comma) — same problem, different mistake. Net result: backfill-see-also doesn't see multi-series books as same-series mates of singletons, and the /series page shows them as a malformed sub-tree. **Implementation outline:** (a) decide canonical YAML shape — `series: ["Discworld #32", "Tiffany Aching #2"]` array is probably cleanest, with the existing `; `-delimited string left as a tolerated input form for back-compat; (b) Book type changes from `series: string | null` to `series: SeriesMembership[]` where each membership is `{name, index}`; (c) update both parsers to one canonical implementation; (d) propagate through `/series` (a book may appear in N sub-trees), `/discover` and the four see_also/tag backfills (any-shared-series counts), `_index.json` schema, and `vault-health` (broken-series-name detection). Touches roughly 7 files plus the index schema migration. The `/series` route, see_also-from-series backfill, and tag-from-peers backfill all silently misbehave today on these books — it's a real bug, not just a polish item. `#bug #schema #series`
...
- Reading-velocity finish projection (full version): once a `pages` schema field is in the vault, extend the home Currently-Reading "days in" caption with a back-of-envelope ETA from historical pace. The days-in caption ships now without ETA. `#feature #stats #currently-reading`
...
- **Patch handler re-serialises whole frontmatter on any change.** `src/lib/mcp/patch.ts:111` drops `frontmatterRaw` and lets the YAML serialiser re-emit from the parsed object whenever any frontmatter key changes. So adding a single tag through `/admin` produces a commit that cosmetically rewrites every other frontmatter key (key order, quoting, scalar vs flow representation) — the diff preview shows only the intended change but the actual commit touches the rest of the block. Semantics are fine; fidelity between preview and commit is the issue. **Fix:** targeted line replacement, mirroring the host-side backfill scripts' pattern (regex-find each changed key's existing line, replace just that line; insert new keys before the closing `---`; delete removed keys). Roughly 80 lines in `patch.ts` plus tests. `#bug #write-surface #mcp #fidelity`
- **Books-repo webhook → `/api/admin/reindex`.** First-deploy required a manual reindex from `/admin` because the store starts empty after Upstash provisioning. Long-term: configure a GitHub webhook on `vhata/books` that POSTs to `/api/admin/reindex` on every push, so the store stays in sync without manual nudges. Setup is one webhook on the books repo + likely a small auth tweak (the proxy currently requires a session cookie; webhook calls won't have one). Either add a webhook-token env var and carve out the route, or accept HMAC-signed payloads. `#tooling #write-surface #mcp #automation`

### Tooling & vault hygiene (brainstormed 2026-05-03)

- **Cron-driven vault hygiene (auto-applied backfills + lint).** The three idempotent backfills (`source`, `see_also`, `tags`) and `vault-lint` exist locally as `make vault-*` targets, but the user still has to remember to run them. Goal: a scheduled job that runs them weekly without prompting and pushes the result back to `vhata/books`. Cleanest shape is a GitHub Actions workflow on **this** repo (`ook`) — that's where the scripts live — that (a) checks out `ook`, (b) checks out `vhata/books` with a write-capable token, (c) runs `make vault-lint` (fail loudly on errors), (d) runs `make vault-backfill-apply`, (e) commits + pushes the result if anything changed. Trigger: weekly `schedule:` cron + `workflow_dispatch`. The push to `vhata/books` then trips the existing webhook that redeploys ook on Vercel, so the public site reflects the cleanup automatically. **Blocked on:** a write-capable secret. The current `BOOKS_DEPLOY_KEY` is read-only by design (Vercel env). Need a fine-grained PAT or a separate write-key + matching webhook entry. The `feral/mcp` branch already plans to provision `GITHUB_BOOKS_PAT` for the same shape of write — if that branch lands first, this workflow can borrow the same secret. `#tooling #vault #automation #ci`
