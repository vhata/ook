...
- **`promote-goodreads.mjs` `SERIES_RE` mishandles double-parens titles** (surfaced 2026-05-10). When Goodreads exports a title like `"We Are Legion (We Are Bob) (Bobiverse, #1)"` — title-with-subtitle-in-parens AND series-in-parens — the lazy `(.+?)` for group 1 matches just `We Are Legion`, and the lazy `(.+?)` for group 2 swallows the rest including the inner closing paren: `We Are Bob) (Bobiverse, #1`. That malformed string ends up sitting in the vault as the `series:` field (currently `vault/We Are Legion/We Are Legion.md`). The runtime parsers (`parseSeriesMemberships`, `readSeriesNames`) are doing the right thing on bad input; the bug is upstream. Two-part fix: (a) manual one-line vault edit to repair the existing bad row (`series: "We Are Bob) (Bobiverse #1"` → `series: "Bobiverse #1"`); (b) harden `SERIES_RE` in `scripts/promote-goodreads.mjs` against nested parens — anchor on the LAST `(...)` rather than lazy-matching from the start, or reject matches with unbalanced inner parens. Bonus: extract a `cleanTitleAndSeries` helper into `scripts/lib/` (matching the `kindle-clippings.mjs` pattern) so it's unit-testable; pin with a vitest case using the real Goodreads title. `#bug #scripts #goodreads #promote`
