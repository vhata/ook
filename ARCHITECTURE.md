...
- **`src/app/`** — Next.js App Router pages (home, `/books/[slug]`, `/log`, `/stats`, `/stats/[year]`, `/series`, `/random`, `/feed.xml`, `/feed.json`, `/api/books/[slug]/notes`). Server components only; data is read at request/build time via the vault reader.
